...
1. **Never run `go build` directly.** Always use `build/build.sh` (handles
   ...
   `src/` drops a stray binary at `src/termulaa` that violates rule 2.
...
5. **Loopback bind is non-negotiable.** The HTTP listener in
   `src/cmd/termulaa/main.go` MUST bind to `127.0.0.1` only. Do not
...
panes in a binary-split layout. State persists to `~/.termulaa/`.

**Build**: `build/build.sh` (or `build/build.sh termulaa 0.1.0`)
**Run**: `./dist/darwin/termulaa-arm64-v0.1.0` (or via
`resources/scripts/run-termulaa.sh`)
...
Seed tests live in `src/cmd/termulaa/*_test.go`. Run from `src/`:
...
- Binary naming at release: `termulaa-<os>-<arch>-v<tag>` (the workflow
  renames `build/build.sh`'s internal `termulaa-...` names).

## Files agents commonly touch

| File | Purpose |
|------|---------|
| `src/cmd/termulaa/main.go` | Entry point, config, security middleware |
| `src/cmd/termulaa/handler.go` | HTTP routes + WebSocket upgraders |
